    ...
    return (
        <div style={{...AppStyles.modal, margin:'auto', width:'auto', border:'none'}}>
            ...
            <div style={{display:'flex', justifyContent:'flex-end'}}>
                ...
                <div style={{display:'flex', justifyContent:'flex-end'}}>
                    {!lobbyId &&<Button enabled={joinLobbyId.length === 3} handler={()=>createOrJoinLobby(joinLobbyId)} text="Join" style={{ padding:'5px'}}/>}
                    {lobbyId ? 
                    <div style={{marginRight:'1em', display:'flex', alignItems:'center'}}>Session ID: {lobbyId}</div> : 
                    <Button enabled={true} handler={()=>{createOrJoinLobby(); setIsHost(true)}} text="Host" style={{ padding:'5px'}}/>}
                    <Button text="Begin" enabled={joinedPlayer && hosting} handler={()=>{clearInterval(interval); sendStartMatch()}} style={{ padding:'5px'}}/>
                    <Button text="Cancel" enabled={true} handler={()=>{onSetLobby('');onShowModal(Modal.NewGame)}} style={{ padding:'5px'}}/>
                </div>
            </div>
        </div>
    )
